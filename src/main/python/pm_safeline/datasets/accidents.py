...
- [load_accidents] : torch 불필요한 GeoDataFrame 로더(수집 파이프라인·지오 연산용).
  수집 파이프라인은 torch 없이 동작해야 하므로 이 함수를 사용한다.
...
if TYPE_CHECKING:
    ...
    _TorchDataset = object  # torch 없이도 클래스 정의 가능
...
def _require_torch():
    try:
        ...
        raise ImportError(
            "AccidentDataset 사용에는 torch 가 필요합니다. torch 없이 GeoDataFrame 만 필요하면 "
            "load_accidents() 를 쓰세요."
        ) from e
...
def _koroad_download_to_raw(
    ...
    *,
    ...
    """
    cfg.ensure_dirs()
    gdf = _koroad_fetch(cfg, kind=kind, years=years, api_key=api_key)
...
def _taas_load(cfg: Config = DEFAULT_CONFIG, *, pm_only: bool = True) -> "gpd.GeoDataFrame":
...
def load_accidents(
    ...
    **kw,
    ...
    """사고 지점을 표준 스키마 GeoDataFrame 으로 로드(torch 불필요).

    download=True 면 소스에서 받아 `data/raw` 에 캐시, False 면 이미 받아둔 캐시를 읽는다.
    """
    if source == "koroad":
        if download:
            gdf = _koroad_download_to_raw(cfg, **kw)
        else:
            cache = cfg.raw_dir / f"koroad_{kw.get('kind', 'motorcycle')}_frequentzone.csv"
            if not cache.exists():
                raise RuntimeError(
...
# --------------------------------------------------------------------------- #
class AccidentDataset(_TorchDataset):
    ...
    인자(torchvision 관례):
        root             : 캐시 위치(기본 cfg.data_dir). 캐시 파일 accidents.gpkg.
        transform        : sample 변환.
    ...
    def __init__(
        ...
        *,
        ...
        self.source = source
        cache = self.root / "accidents.gpkg"

        if download:
            gdf = load_accidents(source, cfg, download=True, pm_only=pm_only)
            self.root.mkdir(parents=True, exist_ok=True)
            gdf.to_file(cache, driver="GPKG")
        elif cache.exists():
            import geopandas as gpd

            gdf = _validate(gpd.read_file(cache))
        else:
            raise RuntimeError(
                f"Dataset not found at {cache}. You can use download=True to download it"
            )
        self.gdf = gdf
