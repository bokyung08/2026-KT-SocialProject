...
├── irl/         # 역강화학습 (비용함수 가중치 학습, §4.2·4.4)
│   ├── route_risk.py    edge 위험 → route 위험 집계(hazard-rate 등, §4.5-3)
│   └── bradley_terry.py 경로쌍 선호 → Bradley-Terry 로 w1~w5 학습
...
| 7. IRL 가중치 학습 | `pm_safeline.irl` | ❌ (numpy/scipy) |
...
수집(1~4)은 메인 venv(torch 불필요)에서, **학습(5~6)은 torch env**에서 돌린다. IRL(7)은 numpy/scipy라 어디서든 실행된다.

```bash
# torch 학습 env (Py3.12) 준비 — 메인 venv(Py3.14)엔 torch 휠이 없어 별도 env 사용
uv venv --python 3.12 .venv-train
uv pip install --python .venv-train torch torchvision scikit-learn pandas pillow numpy scipy ipykernel
.venv-train/Scripts/python -m ipykernel install --user --name pm-train

# 노트북 실행(커널 pm-train): teacher 학습 / IRL
#   src/test/notebooks/train_teacher.ipynb  (실 manifest 필요 → 먼저 collect)
#   src/test/notebooks/train_irl.ipynb      (합성 데모 → 학습된 w1~w5 출력)
```
...
- **IRL**: teacher가 매긴 edge 위험 → `irl.route_risk` 집계 → 경로쌍 선호 → `irl.bradley_terry.fit_bradley_terry` 로 w1~w5 학습 → 서버 `PmCostWeights` 에 주입.
